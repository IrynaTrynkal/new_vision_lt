...
export const OfferCard = ({
    ...
    return (
        <div className="text-ivory prepc:mx-0 tab:aspect-[435/327] tab:max-w-[435px] prepc:w-[32%] mx-auto aspect-[288/194] max-w-80 overflow-hidden rounded bg-center">
            ...
            <div
                className={`tab:p-3 relative flex w-full flex-col justify-between bg-cover bg-center p-2 ${data.discountShortData?.darken ? "bg-blend-multiply" : ""} ${data.discountShortData?.premium ? "tab:h-[calc(100%-62px)] h-[calc(100%-40px)]" : "h-full"}`}
                style={{
            ...
            >
                <div>
                    ...
                    {data.discountShortData?.shortText && (
                        <p className="tab:text-lg tab:leading-[22px] text-sm leading-4 backdrop-blur-[8px]">
                            {data.discountShortData.shortText}
                        </p>
                    )}
                </div>
